"""
FileName: Largest product in a grid
Author: N.G 29.3.21
Purpose: Find the greatest product of four adjacent numbers in the same direction
...
def main():
    ...
    largest_product = 0
    for mini_grid in range(GRID_SIZE - 1):
        for number in range(GRID_SIZE - 1):
            if (number + 3) < GRID_SIZE:
                product = 1
                for num in range(ADJACENT):
                    product *= int(grid[mini_grid][number + num])
            ...
            if (mini_grid + 3) < GRID_SIZE:
                product = 1
                for num in range(ADJACENT):
                    product *= int(grid[mini_grid + num][number])
                largest_product = product if product > largest_product else largest_product
            ...
            if (number + 3) < GRID_SIZE and (mini_grid - 3) >= 0:
                product = 1
                for num in range(ADJACENT):
                    product *= int(grid[mini_grid - num][number + num])
                largest_product = product if product > largest_product else largest_product

            if (number + 3) < GRID_SIZE and (mini_grid + 3) < GRID_SIZE:
                product = 1
                for num in range(ADJACENT):
                    product *= int(grid[mini_grid + num][number + num])
                largest_product = product if product > largest_product else largest_product
